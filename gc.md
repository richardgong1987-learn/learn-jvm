...
垃圾回收执行回收时,会发生stop the world的原因是因为会发生复制算法,并,移动内存,所以为了安全的移动内存,需要暂停所有线程活动
...
-XX:+UseSerialGC  它等于  Serial回收器+SerialOld回收器

Serial是使用"复制算法",SerialOld是使用"标记+整理算法"
...
ParallelOldGC老年代算法都是"标记+整理算法"



**可以通过**:-XX:ParallelGCThreads=n控制线程数目



1.  多线程
2. 堆内存较大,多核cpu
3. 单位时间内所花的stop the world的时间最短
...
-XX:UserConcMarkSweepGC 它等于-XX:+UseParNewGC - SerialOld
...
1.  多线程
2. 堆内存较大,多核cpu
3. 单次Stop the world尽可能短,跟用户线程并行,会跟用户抢线程
...
ZGC jdk12引入的. 超低延迟回收器

parallelGC 是hotspot用的
